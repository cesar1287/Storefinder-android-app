...
public class Queries {
	...
	public void deleteNews(int news_id) {
		db = dbHelper.getWritableDatabase();
		db.delete("news", "news_id = " + String.valueOf(news_id), null);
		db.close();
	}

	public void deletePhoto(int photo_id) {
		db = dbHelper.getWritableDatabase();
		db.delete("photos", "photo_id = " + String.valueOf(photo_id), null);
		db.close();
	}
	...
	public void deleteStore(int store_id) {
		db = dbHelper.getWritableDatabase();
		db.delete("stores", "store_id = " + String.valueOf(store_id), null);
		db.close();
	}

	public void deleteFavorite(int store_id) {
		db = dbHelper.getWritableDatabase();
		db.delete("favorites", "store_id = " + String.valueOf(store_id), null);
		db.close();
	}
	...
	public Favorite getFavoriteByStoreId(int storeId) {
		...
		Favorite entry = null;
		String sql = String.format("SELECT * FROM favorites WHERE store_id = %s", String.valueOf(storeId));
	...
	public News getNewsByNewsId(int newsId) {
		...
		String sql = String.format("SELECT * FROM news WHERE news_id = %s", String.valueOf(newsId));
		Cursor mCursor = db.rawQuery(sql, null);
		mCursor.moveToFirst();
	...
	public ArrayList<Store> getStoresByCategoryId(int categoryId) {
		...
		String sql = String.format("SELECT * FROM stores WHERE category_id = %s", String.valueOf(categoryId));
		Cursor mCursor = db.rawQuery(sql, null);
		mCursor.moveToFirst();
	...
	public Store getStoresByStoreId(int storeId) {
		...
		String sql = String.format("SELECT * FROM stores WHERE store_id = %s", String.valueOf(storeId));
		Cursor mCursor = db.rawQuery(sql, null);
		mCursor.moveToFirst();
	...
	public Photo getPhotoByStoreId(int storeId) {
		Photo entry = null;
		db = dbHelper.getReadableDatabase();
		String sql = String.format("SELECT * FROM photos WHERE store_id = %s ORDER BY photo_id ASC", String.valueOf(storeId));
		Cursor mCursor = db.rawQuery(sql, null);
		mCursor.moveToFirst();
		if (!mCursor.isAfterLast()) {
			do {
	...
	public ArrayList<Photo> getPhotosByStoreId(int storeId) {
		...
		String sql = String.format("SELECT * FROM photos WHERE store_id = %s", String.valueOf(storeId));
		Cursor mCursor = db.rawQuery(sql, null);
		mCursor.moveToFirst();
